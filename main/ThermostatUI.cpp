...
void ThermostatUI::doBegin() {
	...
	lv_image_create_radial_dsc(&_radialGradientDsc, lv_color_black(),
		lv_color_mix(_arcActiveColor, lv_color_black(), (int)(255 * 0.15f)));
...
void ThermostatUI::doRender(lv_obj_t* parent) {
	...
	_radialGradientImage = lv_image_create(parent);
	lv_image_set_src(_radialGradientImage, &_radialGradientDsc);
	...
	_modeButton = createTemperatureButton(parent, "", pw(BUTTON_MODE_X_OFFSET), pw(BUTTON_MODE_Y_OFFSET), pw(BUTTON_RADIUS));
...
lv_obj_t* ThermostatUI::createTemperatureButton(lv_obj_t* parent, const char* image, int x, int y, int radius) {
	const auto button = lv_button_create(parent);
	...
	lv_obj_set_style_bg_image_src(button, image, 0);
...
void ThermostatUI::createArcControl(lv_obj_t* parent) {
	...
	lv_obj_remove_flag(_setpointOuterCircle, LV_OBJ_FLAG_SCROLLABLE);
	...
	lv_obj_set_style_bg_color(_setpointOuterCircle, _notHeatingColor, LV_PART_MAIN);
	lv_obj_set_style_border_color(_setpointOuterCircle, _notHeatingColor, LV_PART_MAIN);

	_setpointCircle = lv_obj_create(parent);
	lv_obj_remove_flag(_setpointCircle, LV_OBJ_FLAG_SCROLLABLE);
...
void ThermostatUI::renderState() {
	...
	if (_state.mode == ThermostatMode::Heat) {
		lv_obj_set_style_bg_image_src(_modeButton, Messages::fireFlameCurvedIcon, 0);
	}
	else {
		lv_obj_set_style_bg_image_src(_modeButton, Messages::powerOffIcon, 0);
	}
	...
	if (_state.mode == ThermostatMode::Off) {
		...
		lv_obj_remove_flag(_setpointOuterCircle, LV_OBJ_FLAG_HIDDEN);
	}
	else {
		lv_obj_remove_flag(_setpointArc, LV_OBJ_FLAG_HIDDEN);
	...
	if (_state.mode == ThermostatMode::Off || setpointOffset <= localTemperatureOffset) {
		...
		lv_obj_remove_flag(_setpointHeatingArc, LV_OBJ_FLAG_HIDDEN);
	...
	auto localTemperatureCircleColor =
		_state.mode == ThermostatMode::Off || setpointOffset < localTemperatureOffset ? _notHeatingColor : lv_color_make(134, 65, 34);
	...
	if (_state.state == ThermostatRunningState::True) {
		lv_obj_remove_flag(_radialGradientImage, LV_OBJ_FLAG_HIDDEN);
	}
	else {
...
void ThermostatUI::handleArcPressed(lv_event_t* e) {
	...
	auto inputDevice = lv_indev_active();
...
void ThermostatUI::handleMode() {
	_msgbox = lv_msgbox_create(nullptr);

	auto heatingButtonLabel = format("%s %s", Messages::fireFlameCurvedIcon, Messages::heating);
	auto heatingButton = lv_msgbox_add_footer_button(_msgbox, heatingButtonLabel.c_str());
	lv_obj_add_event_cb(
		heatingButton, [](auto e) { SELF(e)->handleSetMode(ThermostatMode::Heat); }, LV_EVENT_CLICKED, this);

	auto offButtonLabel = format("%s %s", Messages::powerOffIcon, Messages::off);
	auto offButton = lv_msgbox_add_footer_button(_msgbox, offButtonLabel.c_str());
	lv_obj_add_event_cb(
		offButton, [](auto e) { SELF(e)->handleSetMode(ThermostatMode::Off); }, LV_EVENT_CLICKED, this);

	auto footer = lv_msgbox_get_footer(_msgbox);

	lv_obj_set_width(_msgbox, pw(70));
	lv_obj_set_style_text_font(heatingButton, NORMAL_FONT, LV_PART_MAIN);
	lv_obj_set_style_text_font(offButton, NORMAL_FONT, LV_PART_MAIN);
	lv_obj_set_style_pad_bottom(footer, lv_dpx(20), LV_PART_MAIN);
	lv_obj_set_height(footer, lv_dpx(85));
}
